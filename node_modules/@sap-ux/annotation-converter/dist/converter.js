...
exports.convert = exports.CONVERTER_ROOT = void 0;
...
exports.CONVERTER_ROOT = Symbol('Converter Root');
/**
...
function mapPropertyPath(converter, propertyPath, fullyQualifiedName, currentTarget, currentTerm) {
    ...
    result[exports.CONVERTER_ROOT] = () => converter.getConvertedOutput();
    (0, utils_1.lazy)(converter, result, '$target', () => resolveTarget(converter, currentTarget, propertyPath.PropertyPath, currentTerm).target);
    return result;
}
function mapAnnotationPath(converter, annotationPath, fullyQualifiedName, currentTarget, currentTerm) {
    ...
    result[exports.CONVERTER_ROOT] = () => converter.getConvertedOutput();
    (0, utils_1.lazy)(converter, result, '$target', () => resolveTarget(converter, currentTarget, result.value, currentTerm).target);
    return result;
}
function mapNavigationPropertyPath(converter, navigationPropertyPath, fullyQualifiedName, currentTarget, currentTerm) {
    ...
    result[exports.CONVERTER_ROOT] = () => converter.getConvertedOutput();
    (0, utils_1.lazy)(converter, result, '$target', () => resolveTarget(converter, currentTarget, navigationPropertyPath.NavigationPropertyPath, currentTerm).target);
    return result;
}
function mapPath(converter, path, fullyQualifiedName, currentTarget, currentTerm) {
    ...
    result[exports.CONVERTER_ROOT] = () => converter.getConvertedOutput();
    (0, utils_1.lazy)(converter, result, '$target', () => resolveTarget(converter, currentTarget, path.Path, currentTerm).target);
    return result;
}
...
function parseRecord(converter, currentTerm, currentTarget, currentProperty, currentSource, annotationRecord, currentFQN) {
    ...
    record[exports.CONVERTER_ROOT] = () => converter.getConvertedOutput();
    for (const propertyValue of annotationRecord.propertyValues) {
        (0, utils_1.lazy)(converter, record, propertyValue.name, () => parseValue(converter, currentTarget, currentTerm, propertyValue.name, currentSource, propertyValue.value, `${currentFQN}/${propertyValue.name}`));
    }
    // annotations on the record
    (0, utils_1.lazy)(converter, record, 'annotations', resolveAnnotationsOnAnnotation(converter, annotationRecord, record));
    if (record.hasOwnProperty('CollectionPath')) {
        (0, utils_1.lazy)(converter, record, 'CollectionPathTarget', () => {
            var _a, _b;
            const entityContainerFQN = (_b = (_a = converter.rawSchema.entityContainers) === null || _a === void 0 ? void 0 : _a[currentSource].fullyQualifiedName) !== null && _b !== void 0 ? _b : converter.rawSchema.entityContainer.fullyQualifiedName;
            return converter.getConvertedEntitySet(entityContainerFQN + '/' + record.CollectionPath);
        });
    }
    if (isDataFieldWithForAction(record)) {
        (0, utils_1.lazy)(converter, record, 'ActionTarget', () => {
            var _a, _b;
...
function convertAnnotation(converter, target, rawAnnotation) {
    ...
    if (!annotation[exports.CONVERTER_ROOT]) {
        annotation[exports.CONVERTER_ROOT] = () => converter.getConvertedOutput();
    }
    ...
    try {
        (0, utils_1.lazy)(converter, annotation, 'annotations', resolveAnnotationsOnAnnotation(converter, rawAnnotation, annotation));
    }
    catch (e) {
...
class Converter {
    ...
    reset() {
        for (const dynamicElementsKey in this.dynamicElements) {
            ...
            const targetObject = this.dynamicElements[dynamicElementsKey].target;
            for (const key of keys) {
                targetObject[key] = utils_1.initialSymbol;
            }
        }
        this.annotationsByTarget = undefined;
    }
    collectDynamic(object, property) {
        var _a;
        var _b;
        ...
        (_a = (_b = this.dynamicElements)[objectName]) !== null && _a !== void 0 ? _a : (_b[objectName] = { target: object, keys: [] });
    ...
    addExtraMetadata(extraMetadata) {
        (0, utils_1.mergeRawMetadata)(this.rawMetadata, extraMetadata);
    }
...
function resolveNavigationPropertyBindings(converter, rawNavigationPropertyBindings) {
    return () => Object.keys(rawNavigationPropertyBindings).reduce((navigationPropertyBindings, bindingPath) => {
        ...
        (0, utils_1.lazy)(converter, navigationPropertyBindings, bindingPath, () => {
            var _a;
...
function createAnnotationsObject(converter, target, rawAnnotations) {
    return rawAnnotations.reduce((vocabularyAliases, annotation) => {
        ...
        if (!vocabularyAliases[vocAlias].hasOwnProperty(vocTermWithQualifier)) {
            ...
            (0, utils_1.lazy)(converter, vocabularyAliases[vocAlias], vocTermWithQualifier, () => converter.getConvertedElement(annotation.fullyQualifiedName, annotation, (converter, rawAnnotation) => convertAnnotation(converter, target, rawAnnotation)));
        }
...
function convertEntityContainer(converter, rawEntityContainer) {
    ...
    (0, utils_1.lazy)(converter, convertedEntityContainer, 'annotations', resolveAnnotations(converter, rawEntityContainer));
    (0, utils_1.lazy)(converter, convertedEntityContainer, 'entitySets', converter.convert(converter.rawSchema.entitySets, convertEntitySet));
    (0, utils_1.lazy)(converter, convertedEntityContainer, 'singletons', converter.convert(converter.rawSchema.singletons, convertSingleton));
    (0, utils_1.lazy)(converter, convertedEntityContainer, 'actionImports', converter.convert(converter.rawSchema.actionImports, convertActionImport));
...
function convertSingleton(converter, rawSingleton) {
    ...
    (0, utils_1.lazy)(converter, convertedSingleton, 'entityType', resolveEntityType(converter, rawSingleton.entityTypeName));
    (0, utils_1.lazy)(converter, convertedSingleton, 'annotations', resolveAnnotations(converter, convertedSingleton));
    const _rawNavigationPropertyBindings = rawSingleton.navigationPropertyBinding;
    (0, utils_1.lazy)(converter, convertedSingleton, 'navigationPropertyBinding', resolveNavigationPropertyBindings(converter, _rawNavigationPropertyBindings));
...
function convertEntitySet(converter, rawEntitySet) {
    ...
    (0, utils_1.lazy)(converter, convertedEntitySet, 'entityType', resolveEntityType(converter, rawEntitySet.entityTypeName));
    (0, utils_1.lazy)(converter, convertedEntitySet, 'annotations', resolveAnnotations(converter, convertedEntitySet));
    const _rawNavigationPropertyBindings = rawEntitySet.navigationPropertyBinding;
    (0, utils_1.lazy)(converter, convertedEntitySet, 'navigationPropertyBinding', resolveNavigationPropertyBindings(converter, _rawNavigationPropertyBindings));
...
function convertEntityType(converter, rawEntityType) {
    ...
    (0, utils_1.lazy)(converter, convertedEntityType, 'annotations', resolveAnnotations(converter, rawEntityType));
    (0, utils_1.lazy)(converter, convertedEntityType, 'keys', converter.convert(rawEntityType.keys, convertProperty));
    (0, utils_1.lazy)(converter, convertedEntityType, 'entityProperties', converter.convert(rawEntityType.entityProperties, convertProperty));
    (0, utils_1.lazy)(converter, convertedEntityType, 'navigationProperties', converter.convert(rawEntityType.navigationProperties, convertNavigationProperty));
    (0, utils_1.lazy)(converter, convertedEntityType, 'actions', () => converter.rawSchema.actions
...
function convertProperty(converter, rawProperty) {
    ...
    (0, utils_1.lazy)(converter, convertedProperty, 'annotations', resolveAnnotations(converter, rawProperty));
    (0, utils_1.lazy)(converter, convertedProperty, 'targetType', () => {
        var _a;
...
function convertNavigationProperty(converter, rawNavigationProperty) {
    ...
    if (convertedNavigationProperty.referentialConstraint.length === 0) {
        ...
        const refConstraints = annotations.find((annotation) => annotation.term === "com.sap.vocabularies.Common.v1.ReferentialConstraint" /* CommonAnnotationTerms.ReferentialConstraint */);
        if (refConstraints) {
            convertedNavigationProperty.referentialConstraint =
                (_c = (_b = refConstraints.collection) === null || _b === void 0 ? void 0 : _b.map((record) => {
                    var _a, _b;
                    const referencedProperty = (_a = record.propertyValues.find((prop) => {
                    ...
                    return {
                        sourceProperty: ((_b = record.propertyValues.find((prop) => {
                            return prop.name === 'Property';
                        })) === null || _b === void 0 ? void 0 : _b.value).PropertyPath,
    ...
    if (!isV4NavigationProperty(rawNavigationProperty)) {
        const associationEnd = (_d = converter.rawSchema.associations
            .find((association) => association.fullyQualifiedName === rawNavigationProperty.relationship)) === null || _d === void 0 ? void 0 : _d.associationEnd.find((end) => end.role === rawNavigationProperty.toRole);
    ...
    (0, utils_1.lazy)(converter, convertedNavigationProperty, 'targetType', resolveEntityType(converter, rawNavigationProperty.targetTypeName));
    (0, utils_1.lazy)(converter, convertedNavigationProperty, 'annotations', resolveAnnotations(converter, rawNavigationProperty));
...
function convertActionImport(converter, rawActionImport) {
    ...
    (0, utils_1.lazy)(converter, convertedActionImport, 'annotations', resolveAnnotations(converter, rawActionImport));
    (0, utils_1.lazy)(converter, convertedActionImport, 'action', () => {
...
function convertAction(converter, rawAction) {
    ...
    if (convertedAction.sourceType) {
        (0, utils_1.lazy)(converter, convertedAction, 'sourceEntityType', resolveEntityType(converter, rawAction.sourceType));
    }
    if (convertedAction.returnType) {
        (0, utils_1.lazy)(converter, convertedAction, 'returnEntityType', resolveEntityType(converter, rawAction.returnType));
        (0, utils_1.lazy)(converter, convertedAction, 'returnTypeReference', () => {
            var _a, _b;
            ...
            return ((_b = (_a = converter.getConvertedEntityType(typeName)) !== null && _a !== void 0 ? _a : converter.getConvertedComplexType(typeName)) !== null && _b !== void 0 ? _b : converter.getConvertedTypeDefinition(typeName));
        });
    }
    (0, utils_1.lazy)(converter, convertedAction, 'parameters', converter.convert(rawAction.parameters, convertActionParameter));
    (0, utils_1.lazy)(converter, convertedAction, 'annotations', () => {
        /*
...
function convertActionParameter(converter, rawActionParameter) {
    ...
    (0, utils_1.lazy)(converter, convertedActionParameter, 'typeReference', () => {
        var _a, _b;
        ...
        return ((_b = (_a = converter.getConvertedEntityType(targetType)) !== null && _a !== void 0 ? _a : converter.getConvertedComplexType(targetType)) !== null && _b !== void 0 ? _b : converter.getConvertedTypeDefinition(targetType));
    });
    (0, utils_1.lazy)(converter, convertedActionParameter, 'annotations', () => {
...
function convertComplexType(converter, rawComplexType) {
    ...
    (0, utils_1.lazy)(converter, convertedComplexType, 'properties', converter.convert(rawComplexType.properties, convertProperty));
    (0, utils_1.lazy)(converter, convertedComplexType, 'navigationProperties', converter.convert(rawComplexType.navigationProperties, convertNavigationProperty));
    (0, utils_1.lazy)(converter, convertedComplexType, 'annotations', resolveAnnotations(converter, rawComplexType));
 ...
 */
function convertEnumMember(converter, rawEnumMember) {
    const convertedEnumMember = rawEnumMember;
    (0, utils_1.lazy)(converter, convertedEnumMember, 'annotations', resolveAnnotations(converter, rawEnumMember));
 ...
 */
function convertEnumType(converter, rawEnumType) {
    const convertedEnumType = rawEnumType;
    (0, utils_1.lazy)(converter, convertedEnumType, 'members', converter.convert(rawEnumType.members, convertEnumMember));
    (0, utils_1.lazy)(converter, convertedEnumType, 'annotations', resolveAnnotations(converter, rawEnumType));
...
function convertTypeDefinition(converter, rawTypeDefinition) {
    ...
    (0, utils_1.lazy)(converter, convertedTypeDefinition, 'annotations', resolveAnnotations(converter, rawTypeDefinition));
...
function convert(rawMetadata) {
    ...
    (0, utils_1.lazy)(converter, convertedOutput, 'entityContainer', converter.convert(converter.rawSchema.entityContainer, convertEntityContainer));
    (0, utils_1.lazy)(converter, convertedOutput, 'entitySets', converter.convert(converter.rawSchema.entitySets, convertEntitySet));
    (0, utils_1.lazy)(converter, convertedOutput, 'singletons', converter.convert(converter.rawSchema.singletons, convertSingleton));
    (0, utils_1.lazy)(converter, convertedOutput, 'entityTypes', converter.convert(converter.rawSchema.entityTypes, convertEntityType));
    (0, utils_1.lazy)(converter, convertedOutput, 'actions', converter.convert(converter.rawSchema.actions, convertAction));
    (0, utils_1.lazy)(converter, convertedOutput, 'complexTypes', converter.convert(converter.rawSchema.complexTypes, convertComplexType));
    (0, utils_1.lazy)(converter, convertedOutput, 'enumTypes', converter.convert(converter.rawSchema.enumTypes, convertEnumType));
    (0, utils_1.lazy)(converter, convertedOutput, 'actionImports', converter.convert(converter.rawSchema.actionImports, convertActionImport));
    (0, utils_1.lazy)(converter, convertedOutput, 'typeDefinitions', converter.convert(converter.rawSchema.typeDefinitions, convertTypeDefinition));
    convertedOutput.resolvePath = function resolvePath(path, startingPoint) {
        const targetResolution = resolveTarget(converter, startingPoint, path);
     ...
     * @returns {void}
     */
    convertedOutput.addValueListWithReferences = function (rawVHMetadata) {
        const converter = convertedOutput.getConverter();
