...
class HybridStore {
    ...
    async read({ entityName, id }) {
        ...
        if (!serialized) {
            ...
            this.logger.debug(`hybrid/read - id: [${id}], filesystem: ${(0, node_util_1.inspect)(serialized)}`);
        }
    ...
    async write({ entityName, id, entity }) {
        ...
        if ((0, utils_1.hasAnyValue)(entity, serializableProps)) {
            this.logger.debug(`hybrid/write - writing serializable properties: ${(0, node_util_1.inspect)(serializable)}`);
            await this.filesystem.write({ entityName, id, entity: serializable });
        }
        else {
            this.logger.debug(`hybrid/write - no serializable properties found in ${(0, node_util_1.inspect)(serializable)}`);
        }
        const sensitiveData = (0, utils_1.pick)(entity, ...sensitiveProps);
        if ((0, utils_1.hasAnyValue)(sensitiveData, sensitiveProps)) {
            ...
            this.logger.debug(`hybrid/write - no sensitive properties found in ${(0, node_util_1.inspect)(entity)}`);
        }
        return entity;
    }
