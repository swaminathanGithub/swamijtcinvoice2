 ...
 */
export const authenticationTypeToProperties: Record<
  AuthenticationType,
  (
    credentials: Credentials,
  ) => OAuth2ClientCredentials | OAuth2UserTokenExchange | undefined
> = {
  [AuthenticationType.OAuth2UserTokenExchange]: (credentials) =>
    credentials.oauth2UserTokenExchange,
  [AuthenticationType.OAuth2ClientCredentials]: (credentials) =>
...
export function strToTokenServiceUrlType(
  typeName: string,
): TokenServiceURLType {
  const typeNameArr: TokenServiceURLType[] = ["Dedicated", "Common"];
  const tokenServiceUrlType = typeNameArr.find(
...
export function destinationInfoToAnswerDestination(
  ...
  if (
    ["OAuth2ClientCredentials", "OAuth2UserTokenExchange"].includes(
      destinationInfo.credentials.authentication,
    )
  ) {
...
function getOAuthProperties(
  credentials: Credentials,
): OAuth2ClientCredentials | OAuth2UserTokenExchange {
  const oauthProperties =
    authenticationTypeToProperties[credentials.authentication](credentials);
  if (!oauthProperties) {
