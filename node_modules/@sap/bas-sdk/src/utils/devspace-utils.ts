...
export async function verifyDefaultLandscape(): Promise<{
  ...
  if (!vscode) {
    try {
      ...
      throw new Error(
        `This functionality can only be used from within the vscode extension context: ${error}`,
      );
    }
  }

  // Get or enable the default landscape
  let defaultLandscape: string | undefined =
  ...
  // Get a valid session, force login if missing or expired
